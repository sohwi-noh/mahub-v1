defmodule SymphonyElixir.Codex.AppServer do
  ...
  defp send_message(port, message) do
    line = Jason.encode!(json_safe_value(message)) <> "\n"
    Port.command(port, line)
  end

  defp json_safe_value(value) when is_binary(value), do: ensure_utf8(value)
  ...
  defp json_safe_value(value) when is_map(value) do
    if Map.has_key?(value, :__struct__) do
      ...
      Map.new(value, fn {key, nested} ->
        {json_safe_key(key), json_safe_value(nested)}
      end)
    end
  end
  ...
  defp json_safe_key(key) when is_binary(key), do: ensure_utf8(key)
  defp json_safe_key(key), do: key

  defp ensure_utf8(value) do
    if String.valid?(value), do: value, else: sanitize_utf8(value, [])
  end

  defp sanitize_utf8(<<>>, acc), do: acc |> Enum.reverse() |> IO.iodata_to_binary()
  defp sanitize_utf8(<<codepoint::utf8, rest::binary>>, acc), do: sanitize_utf8(rest, [<<codepoint::utf8>> | acc])
  defp sanitize_utf8(<<_byte, rest::binary>>, acc), do: sanitize_utf8(rest, [<<0xEF, 0xBF, 0xBD>> | acc])
